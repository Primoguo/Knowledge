...
- [Models/KnowledgeEntry.swift](file://Models/KnowledgeEntry.swift)
- [Models/KnowledgeChat.swift](file://Models/KnowledgeChat.swift)
- [Models/CompanionChat.swift](file://Models/CompanionChat.swift)
...
- [Services/KnowledgeService.swift](file://Services/KnowledgeService.swift)
...
- [Views/KnowledgeListView.swift](file://Views/KnowledgeListView.swift)
- [Views/KnowledgeDetailView.swift](file://Views/KnowledgeDetailView.swift)
</cite>

## 更新摘要
**所做更改**
- 新增 KnowledgeEntry 和 KnowledgeChat 两个 SwiftData 模型的详细说明
- 更新 SwiftData 容器注册架构，展示新模型的集成方式
- 扩展知识库功能的数据流和服务层交互说明
- 新增知识库相关的视图层使用示例和最佳实践
...
- **新增** KnowledgeEntry 和 KnowledgeChat 知识库相关模型的设计与实现
- 数据持久化策略、迁移方案与性能优化建议
- 数据模型的使用示例与最佳实践
...
- Models：领域数据模型与配置
- Services：AI 摘要、语音合成、音频会话、知识库管理等外部能力封装
...
TM["ThemeMode.swift"]
KE["KnowledgeEntry.swift"]
KC["KnowledgeChat.swift"]
CC["CompanionChat.swift"]
end
subgraph "服务层"
AIS["AISummaryService.swift"]
CVS["CosyVoiceService.swift"]
SS["SpeechService.swift"]
KS["KnowledgeService.swift"]
end
App --> Doc
App --> KE
App --> KC
...
- [Services/KnowledgeService.swift:8-16](file://Services/KnowledgeService.swift#L8-L16)
...
- **新增** KnowledgeEntry：知识库条目实体，支持多种来源类型和AI分类
- **新增** KnowledgeChat：知识库AI对话持久化模型，按知识条目存储对话历史
- CompanionChat：文档伴读对话持久化模型（已有功能）
...
- [Models/KnowledgeEntry.swift:64-112](file://Models/KnowledgeEntry.swift#L64-L112)
- [Models/KnowledgeChat.swift:6-26](file://Models/KnowledgeChat.swift#L6-L26)
- [Models/CompanionChat.swift:21-41](file://Models/CompanionChat.swift#L21-L41)
...
+ask(question,knowledgeContent) String
+loadConversation(entryId,context) [ChatEntry]
...
KnowledgeService --> KnowledgeEntry : "操作"
KnowledgeService --> KnowledgeChat : "管理对话"
KnowledgeChat --> ChatEntry : "编码/解码"
```
...
- [Models/KnowledgeEntry.swift:64-112](file://Models/KnowledgeEntry.swift#L64-L112)
- [Models/KnowledgeChat.swift:6-26](file://Models/KnowledgeChat.swift#L6-L26)
- [Models/CompanionChat.swift:5-18](file://Models/CompanionChat.swift#L5-L18)
- [ViewModels/SpeakerViewModel.swift:8-54](file://ViewModels/SpeakerViewModel.swift#L8-L54)
- [Services/AISummaryService.swift:25-34](file://Services/AISummaryService.swift#L25-L34)
- [Services/SpeechService.swift:5-23](file://Services/SpeechService.swift#L5-L23)
- [Services/KnowledgeService.swift:8-114](file://Services/KnowledgeService.swift#L8-L114)
...
### KnowledgeEntry 知识库条目模型（新增）
- 数据来源与分类
  - KnowledgeSource 枚举：支持文档摘要沉淀、语音速记沉淀、手动添加三种来源
  - KnowledgeCategory 枚举：支持会议纪要、创意速记、To-do List、通用知识四种分类
  - 每个枚举项都提供 displayName、iconName 等 UI 相关属性
- 核心字段
  - id：唯一标识符
  - title：知识标题
  - content：正文内容
  - sourceRaw/categoryRaw：持久化的原始字符串值
  - sourceDocumentId：关联的来源文档ID（用于摘要沉淀场景）
  - createdAt/updatedAt：时间戳管理
- 计算属性
  - source/category：从原始字符串派生的枚举类型
  - preview：前100字预览，用于列表展示
- 生命周期管理
  - 默认值策略：source默认为manual，category默认为general
  - 时间戳自动更新：创建时设置初始时间，更新时维护更新时间
- 与现有系统的集成
  - 可通过 sourceDocumentId 关联到 Document 实体
  - 支持从文档摘要自动生成知识条目

章节来源
- [Models/KnowledgeEntry.swift:6-112](file://Models/KnowledgeEntry.swift#L6-L112)

### KnowledgeChat 知识库对话模型（新增）
- 设计目标
  - 按知识条目存储AI对话历史
  - 复用现有的 ChatEntry 数据结构
  - 提供便捷的JSON编解码接口
- 核心字段
  - entryId：关联的知识条目ID
  - messagesData：[ChatEntry] 的JSON编码数据
  - updatedAt：最后更新时间
- 便捷接口
  - entries：计算属性，提供JSON数据的自动编解码
  - 初始化时支持传入ChatEntry数组
- 与 KnowledgeService 的协作
  - 通过 KnowledgeService 进行CRUD操作
  - 支持对话历史的加载、保存、清理等操作
...
- CompanionChat
  - 文档伴读对话持久化模型，与 KnowledgeChat 类似但针对文档场景
...
- 低耦合与单一职责
  ...
  - **新增** KnowledgeEntry 与 KnowledgeChat 专注于知识库数据管理
  - **新增** KnowledgeService 提供统一的知识库AI对话服务
- 外部依赖
  - SwiftData：所有实体的持久化与查询
  - UserDefaults：VoiceConfig 与克隆音色的轻量持久化
  - URLSession：AI 摘要与语音合成的网络请求
  - AVFoundation：系统 TTS 播放与状态回调
  - **新增** ServerAPIClient：知识库AI对话的网络请求
...
SS --> AVF["AVFoundation"]
KE["KnowledgeEntry.swift"] --> SD
KC["KnowledgeChat.swift"] --> SD
KS["KnowledgeService.swift"] --> KE
KS --> KC
KS --> SP["ServerAPIClient"]
SP --> Net
```
...
- 内存与 I/O
  ...
  - **新增** 知识库对话历史限制在20条以内，避免无限增长
- 查询优化
  - 使用 @Query 排序与过滤，减少不必要的全量扫描
  - **新增** KnowledgeListView 支持分类筛选和搜索，提升大数据集下的性能
...
- [Views/DocumentListView.swift:9](file://Views/DocumentListView.swift#L9)
- [Services/KnowledgeService.swift:63-68](file://Services/KnowledgeService.swift#L63-L68)
- [Views/KnowledgeListView.swift:43-61](file://Views/KnowledgeListView.swift#L43-L61)
...
- [Views/KnowledgeDetailView.swift:227-244](file://Views/KnowledgeDetailView.swift#L227-L244)
...
- **新增** KnowledgeEntry 和 KnowledgeChat 构建了完整的知识库管理体系
- **新增** KnowledgeService 提供了专业的知识库AI对话服务
- 通过 UserDefaults 与 SwiftData 的组合，兼顾轻量配置与结构化数据的持久化
- 整体架构支持从文档管理到知识沉淀再到AI对话的完整工作流
...
- **新增** 创建知识条目
  - 从文档摘要生成知识条目，设置适当的来源和分类
  - 参考路径：[Models/KnowledgeEntry.swift:93-111](file://Models/KnowledgeEntry.swift#L93-L111)
- **新增** 知识库对话管理
  - 使用 KnowledgeService 进行问答操作，自动管理对话历史
  - 参考路径：[Services/KnowledgeService.swift:52-71](file://Services/KnowledgeService.swift#L52-L71)
...
- 性能优化
  ...
  - **新增** 知识库对话历史限制大小，避免内存占用过大
- 可扩展性
  - 为未来关系建模预留字段（如外键、版本号）
  - 抽象合成器协议，便于替换与测试
  - **新增** 知识来源和分类系统支持灵活扩展
- **新增** 知识库使用建议
  - 合理设置知识条目的分类，便于后续管理和检索
  - 利用预览功能优化列表展示性能
  - 定期清理不需要的对话历史，保持数据库整洁
...
- [ViewModels/SpeakerViewModel.swift:175-203](file://ViewModels/SpeakerViewModel.swift#L175-L203)
- [ViewModels/SpeakerViewModel.swift:249-260](file://ViewModels/SpeakerViewModel.swift#L249-L260)
- [Models/KnowledgeEntry.swift:93-111](file://Models/KnowledgeEntry.swift#L93-L111)
- [Services/KnowledgeService.swift:52-71](file://Services/KnowledgeService.swift#L52-L71)
- [Views/KnowledgeListView.swift:43-61](file://Views/KnowledgeListView.swift#L43-L61)
- [Views/KnowledgeDetailView.swift:205-251](file://Views/KnowledgeDetailView.swift#L205-L251)