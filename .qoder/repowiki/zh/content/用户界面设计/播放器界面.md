...
## 更新摘要
**变更内容**
- 新增AI伴读功能入口按钮，在工具栏左侧展示聊天气泡图标
- 实现伴读视图作为sheet覆盖层，支持边听边问的交互式对话
- 集成通义千问AI服务，提供基于当前朗读上下文的智能问答
- 增加伴读状态管理和对话历史维护
...
## 简介
本文件面向"有声阅读器"的播放器界面，系统性说明 PlayerView 与 PlayerControlsView 的核心功能、用户交互设计、播放状态同步、后台播放与锁屏控制集成，并给出语音选择、语速调节、音量控制的 UI 交互说明。**最新更新**：新增AI伴读功能，支持在阅读过程中通过聊天气泡图标进入交互式问答界面，AI会基于当前朗读上下文提供智能回答。同时覆盖播放队列管理、循环播放与随机播放的实现现状与扩展建议，以及自定义样式和新增控制功能的扩展方法。
...
**图表来源**
- [Views/PlayerView.swift:1-187](file://Views/PlayerView.swift#L1-L187)
...
- [ViewModels/SpeakerViewModel.swift:1-378](file://ViewModels/SpeakerViewModel.swift#L1-L378)
...
## 核心组件
- PlayerView：播放器主界面，负责文档信息展示、高亮文本滚动跟随、进度条与底部控制区布局，以及摘要弹窗入口。**新增**：工具栏左侧AI伴读按钮，点击后以sheet形式打开伴读对话界面。
- PlayerControlsView：播放控制按钮与快捷语速切换，提供播放/暂停、前进后退、快速语速预设等交互。
- CompanionView：AI伴读对话界面，支持实时问答、快捷问题、对话历史管理与自动滚动。
- SpeakerViewModel：统一编排播放流程、状态同步、远程控制绑定、配置持久化与错误降级。**新增**：伴读状态管理、对话消息维护、上下文提取与AI问答接口。
...
- 空状态：无文档时展示引导文案与图标。
- **新增**：AI伴读sheet覆盖层，打开时自动暂停朗读，退出时恢复播放。
...
- **新增**：伴读功能状态管理，包括对话消息数组、询问状态标志、播放暂停标记等。
- **新增**：AI问答接口，支持提取当前朗读上下文并调用AI服务获取智能回答。
...
- **新增**：AI伴读功能独立于播放核心逻辑，通过专用服务提供，不影响现有播放功能。
- 潜在风险：
...
- 网络请求：
  ...
  - **新增**：AI伴读请求应加入超时处理和错误重试机制，避免长时间等待。
- 音频资源：
  - 临时文件及时清理，避免磁盘增长；长任务注意取消与状态一致性。
- **新增**：伴读对话历史管理，限制最大对话轮数防止内存增长。
...
## 结论
播放器界面以清晰的 MVVM 分层与协议抽象实现了双引擎可插拔的 TTS 能力，结合系统媒体中心提供了完整的后台播放与锁屏控制体验。**最新更新**：新增的AI伴读功能为用户提供了智能化的阅读辅助体验，通过聊天气泡图标便捷访问，支持边听边问的交互式学习模式。当前版本聚焦单文档朗读，后续可扩展队列、循环与随机播放，并完善音量控制 UI。整体架构具备良好的扩展性与可维护性，AI伴读功能的模块化设计确保了与核心播放逻辑的解耦。
...
- **新增**：伴读界面样式可通过修改 CompanionView 中的气泡样式、颜色主题与动画效果进行个性化定制。
...
- 示例参考路径：
  ...
  - 配置更新与持久化：[ViewModels/SpeakerViewModel.swift:1-378](file://ViewModels/SpeakerViewModel.swift#L1-L378)
