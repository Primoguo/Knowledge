...
## 更新摘要
**变更内容**
- **文本高亮系统重大重构**：采用全新的窗口式高亮方法替代基于交集的范围计算，实现精确的"阅读光标"效果
- **高亮样式优化**：当前朗读位置前方约12个字符以蓝色和粗体显示，默认文本使用常规字重而非半粗体，提升对比度
- **性能改进**：通过防抖机制减少UI频繁更新，提升长文档播放流畅度
...
**图表来源**
- [Views/PlayerView.swift:1-391](file://Views/PlayerView.swift#L1-L391)
- [Views/PlayerControlsView.swift:1-79](file://Views/PlayerControlsView.swift#L1-L79)
...
- [ViewModels/SpeakerViewModel.swift:1-500](file://ViewModels/SpeakerViewModel.swift#L1-L500)
...
- **文本高亮系统重大升级**：**全新架构** - 采用窗口式高亮方法，在当前位置前方约12个字符范围内创建精确的"阅读光标"效果，使用蓝色和粗体字体突出显示，默认文本使用常规字重以提升对比度。
...
### PlayerView 分析与交互设计
**重大架构升级与最新UI调整**：实现段落级渲染、精准滚动定位、AI功能栏和付费墙集成，**重要升级**：采用自定义ProgressBarView提供增强的进度条体验，**视觉升级**：使用新的品牌色渐变背景提升视觉效果，**状态修复**：完善了播放完成状态的处理逻辑，**文本高亮系统重大升级**：采用全新的窗口式高亮方法，在当前位置前方约12个字符范围内创建精确的"阅读光标"效果
...
- **智能高亮算法**：**重大升级** - 采用全新的窗口式高亮方法，为每个段落单独计算高亮窗口，仅对当前位置前方约12个字符应用蓝色前景色和粗体字体，创建精确的"阅读光标"效果。默认文本使用常规字重而非半粗体，提供更好的对比度。
...
RenderParagraphs --> WindowHighlight["窗口式高亮算法<br/>当前位置前方12字符"]
WindowHighlight --> ScrollSync["onChange监听段落变化"]
...
### 文本高亮系统重大升级
**全新窗口式高亮架构**：采用精确的"阅读光标"效果，在当前位置前方约12个字符范围内创建高亮显示

- **窗口式高亮算法**：**重大架构升级** - 摒弃传统的基于交集的范围计算方法，采用窗口式高亮策略。在当前朗读位置前方约12个字符范围内创建高亮窗口，模拟真实的"阅读光标"效果。
- **精确高亮范围**：通过 `hlStart = readPos` 和 `hlEnd = readPos + highlightLen` 计算高亮窗口的起始和结束位置，其中 `highlightLen = 12` 定义了高亮字符数量。
- **窗口交集检测**：使用 `hlStart < paraEnd && hlEnd > globalOffset` 条件判断高亮窗口是否与当前段落存在交集，确保只在相关段落内应用高亮样式。
- **局部坐标转换**：将全局坐标转换为段落内局部坐标，通过 `localStart = max(0, hlStart - globalOffset)` 和 `localEnd = min((para.text as NSString).length, hlEnd - globalOffset)` 计算局部高亮范围。
- **视觉样式优化**：高亮部分使用蓝色前景色（`.blue`）和粗体字体（`weight: .bold`），默认文本使用常规字重（`weight: .regular`），提供更好的视觉对比度。
- **性能优化**：仅在段落级别计算高亮范围，避免全量文本重绘，显著提升长文档的渲染性能。
...
HighlightSystem --> ParagraphInfo : "使用"
HighlightSystem --> AttributedString : "应用样式"
```

**图表来源**
- [Views/PlayerView.swift:315-354](file://Views/PlayerView.swift#L315-L354)
...
**图表来源**
- [Services/AISummaryService.swift:6-11](file://Services/AISummaryService.swift#L6-11)
...
- **潜在风险**：
  ...
  - **窗口式高亮性能**：虽然相比传统方法有性能提升，但在超长文档中仍需注意高亮计算的开销。
...
- **窗口式高亮系统优化**：**重大升级**
  - **精确窗口计算**：通过固定长度的高亮窗口（12个字符）减少计算复杂度，避免全量文本扫描。
  - **段落级优化**：仅在相关段落内应用高亮样式，避免不必要的字符串操作。
  - **视觉对比度提升**：默认文本使用常规字重，高亮部分使用粗体，提供更好的可读性。
  - **颜色方案优化**：使用蓝色高亮色，在大多数主题背景下都有良好的对比度。
...
- **高亮不同步**：
  ...
  - **窗口式高亮问题**：检查高亮窗口计算逻辑，确认 hlStart 和 hlEnd 的计算是否正确。
- **段落滚动异常**：
...
- **文本高亮系统问题**：**新增排查项**
  - **窗口式高亮计算**：检查 highlightedParagraph 方法中高亮窗口的计算逻辑。
  - **颜色显示问题**：确认蓝色高亮色在不同主题下的显示效果。
  - **字体权重问题**：验证默认文本使用常规字重，高亮部分使用粗体的配置。
  - **性能问题**：检查超长文档的高亮计算性能，必要时优化窗口大小。
...
- **高亮样式定制**：**重大升级** - 可通过修改 highlightedParagraph 方法中的高亮窗口大小、颜色和字体权重来自定义阅读光标效果。
...
- **示例参考路径**：
  - 控制按钮与快捷语速：[Views/PlayerControlsView.swift:1-79](file://Views/PlayerControlsView.swift#L1-L79)
  - 配置更新与持久化：[ViewModels/SpeakerViewModel.swift:1-500](file://ViewModels/SpeakerViewModel.swift#L1-L500)
...
### 文本高亮系统扩展
**重大升级**：扩展文本高亮系统的功能和定制选项

- **高亮窗口大小定制**：可通过修改 highlightLen 变量调整高亮字符数量，适应不同阅读习惯。
- **颜色方案定制**：可自定义高亮颜色，支持深色模式适配和主题切换。
- **字体权重定制**：可调整默认文本和高亮部分的字体权重，优化可读性。
- **动画效果增强**：可为高亮变化添加平滑过渡动画，提升视觉体验。
- **性能优化**：对于超长文档，可实现增量高亮更新，避免全量重绘。
- **自定义高亮样式**：可支持多种高亮样式，如背景色高亮、边框高亮等。
