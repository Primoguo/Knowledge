...
## 更新摘要
**所做更改**
- 新增 Vnote 录音功能的音频会话管理支持
- 扩展 AudioSessionService 以支持录音场景的会话配置
- 添加录音与播放模式切换的管理逻辑
- 完善麦克风权限处理和错误处理机制
- 更新音频中断处理策略以支持录音场景
...
**更新** 新增了 Vnote 语音速记功能的音频录制支持，扩展了音频会话管理以支持录音场景，包括录音模式配置、麦克风权限管理和录音与播放模式的智能切换。
...
- Services/AudioRecorderService.swift：新增的录音服务，封装 AVAudioRecorder 并提供录音功能
...
AudioRecorder["AudioRecorderService<br/>录音功能封装"]
...
- [AudioRecorderService.swift:1-140](file://Services/AudioRecorderService.swift#L1-L140)
...
- **AudioRecorderService**：**新增** 录音服务，封装 AVAudioRecorder 提供完整的录音功能，包括权限请求、录音控制、音量监控和文件管理。
...
## 架构总览
下图展示了从用户交互到系统能力的完整链路：ViewModel 驱动播放，AudioSessionService 管理会话，NowPlayingService 更新锁屏与控制事件，最终由系统 TTS 输出音频。**新增的 Vnote 录音流程通过 AudioRecorderService 管理录音会话，并在完成后自动切换回播放模式。**
...
### AudioRecorderService：录音功能封装
- **新增功能**
  - 完整的录音生命周期管理：开始、停止、取消录音
  - 麦克风权限请求和处理
  - 实时音量监控和波形显示支持
  - 录音文件管理和存储
  - 录音与播放模式的自动切换
...
- **录音阶段**
  - **新增** 用户开始录音时，AudioRecorderService 直接配置录音模式并激活会话
  - **新增** 录音结束时自动切换回播放模式，保持应用音频体验的一致性
...
- 后台权限
  ...
  - **新增** NSMicrophoneUsageDescription 声明麦克风使用目的，符合隐私要求。
  - **新增** NSSpeechRecognitionUsageDescription 声明语音识别用途。
- 锁屏界面
...
VM --> SP["SpeechService"]
VR["VnoteRecorderView"] --> AR["AudioRecorderService"]
AR --> AS
...
## 结论
Knowledge 应用的音频会话管理采用分层与门面模式，将 AVAudioSession 的管理集中在 AudioSessionService，通过 SpeakerViewModel 编排播放流程，并由 NowPlayingService 完成锁屏与远程控制集成。**新增的 Vnote 录音功能通过 AudioRecorderService 提供了完整的录音支持，实现了录音与播放模式的智能切换。**

当前实现了基础的后台播放与系统媒体控制，**并成功集成了录音功能**。建议在后续版本中加入完整的音频中断监听与恢复机制，进一步提升鲁棒性与用户体验，特别是针对录音场景的中断处理和数据保护。

**更新** 随着 Vnote 功能的加入，音频会话管理现在需要同时支持播放和录音两种模式，这要求更加精细化的会话状态管理和模式切换逻辑。未来的改进方向包括：
